<?php if (!isset($_SESSION)) session_start();


if(!$_POST) exit();

		if (!defined("PHP_EOL")) define("PHP_EOL", "\r\n");

		$subject     	= 'Online overenie obsadenosti - rezervácia ubytovania - Penzión Darinka';
		$start    	= $_POST['start'];
		$end    	= $_POST['end'];
		$izba     	= $_POST['izba'];
		$pocetdospelych     	= $_POST['pocetdospelych'];
		$pocetdeti     	= $_POST['pocetdeti'];
		$name     	= $_POST['name'];
		$email    	= $_POST['email'];
		$phone    	= $_POST['phone'];
    	$comments 	= $_POST['message'];
    	$error		= '';

		// Important Variables

		if(trim($name) == '') {
        	echo '<div class="error_message">Error. Enter your name.</div>';
			exit();
        } else if(trim($email) == '') {
        	echo '<div class="error_message">Error. Enter your e-mail.</div>';
			exit();
        } else if(!isEmail($email)) {
        	echo '<div class="error_message">Error. Enter your VALID e-mail.</div>';
			exit();
        }

        if($error == '') {

			if(get_magic_quotes_gpc()) {
            	$comments = stripslashes($comments);
				$subject = stripslashes($subject);
            }


         // Configuration option.
		 // Enter the email address that you want to emails to be sent to.
		 // Example $address = "joe.doe@yourdomain.com";

         //$address = "example@themeforest.net";
         $address = "bystriansky.d@gmail.com";


         // Configuration option.
         // i.e. The standard subject will appear as, "You've been contacted by John Doe."

         // Example, $e_subject = '$name . ' has contacted you via Your Website.';

         $e_subject = $subject;


         // Configuration option.
		 // You can change this if you feel that you need to.
		 // Developers, you may wish to add more fields to the form, in which case you must be sure to add them here.

		 $esubject = "$subject" . PHP_EOL . PHP_EOL;
		 $e_body = "Príchod: $start " . PHP_EOL .
		 			"Odchod: $end " . PHP_EOL .
		 			"Počet dospelých: $pocetdospelych " . PHP_EOL .
		 			"Počet detí: $pocetdeti " . PHP_EOL .
		 			"Typ izby: $izba " . PHP_EOL .
		 			"Meno a priezvisko: $name " . PHP_EOL .
		 			"Telefón: $phone " . PHP_EOL . PHP_EOL;
		 $e_content = "Špeciálne prianie: \"$comments\"";
		 $e_reply = "E-mail: $email" . PHP_EOL . PHP_EOL;

         $msg = wordwrap($esubject . $e_body . $e_reply . $e_content,100);
		 $msg2 = wordwrap("Pomyślnie zaakceptowaliśmy Twoją prośbę o weryfikację dostępności." . PHP_EOL . "Skontaktujemy się z Tobą wkrótce.",100);

         $headers = "From: $email" . PHP_EOL;
		 $headers .= "Reply-To: $email" . PHP_EOL;
		 $headers .= "MIME-Version: 1.0" . PHP_EOL;
		 $headers .= "Content-type: text/plain; charset=utf-8" . PHP_EOL;
		 $headers .= "Content-Transfer-Encoding: quoted-printable" . PHP_EOL;

		 $headers2 = "From: $address" . PHP_EOL;
		 $headers2 .= "Reply-To: $address" . PHP_EOL;
		 $headers2 .= "MIME-Version: 1.0" . PHP_EOL;
		 $headers2 .= "Content-type: text/plain; charset=utf-8" . PHP_EOL;
		 $headers2 .= "Content-Transfer-Encoding: quoted-printable" . PHP_EOL;

		 //if ($_GET['action'] == 'p01') {

		 $success = mail($address, $e_subject, $msg, $headers);
		 $success2 = mail($email, $e_subject, $msg2, $headers2);

		 //} else if ($_GET['action'] == 'p02') {

		 //$success = mail($address, $e_subject, $msg, $headers);

		 //} else if ($_GET['action'] == 'p03') {

		 //$success = mail($address, $e_subject, $msg, $headers);

		 //} else if ($_GET['action'] == 'p04') {

		 //$success = mail($address, $e_subject, $msg, $headers);

		 //} else if ($_GET['action'] == 'p05') {

		 //$success = mail($address, $e_subject, $msg, $headers);

		 //}



	}

function isEmail($email) { // Email address verification, do not edit.

return(preg_match("/^[-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+(ad|ae|aero|af|ag|ai|al|am|an|ao|aq|ar|arpa|as|at|au|aw|az|ba|bb|bd|be|bf|bg|bh|bi|biz|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|com|coop|cr|cs|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|edu|ee|eg|eh|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gh|gi|gl|gm|gn|gov|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|in|info|int|io|iq|ir|is|it|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|me|mg|mh|mil|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|museum|mv|mw|mx|my|mz|na|name|nc|ne|net|nf|ng|ni|nl|no|np|nr|nt|nu|nz|om|org|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|pro|ps|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tf|tg|th|tj|tk|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)$|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$/i",$email));

// remove all session variables
session_unset();

// destroy the session
session_destroy();
}

header('Location: kontakt.php');
die();

?>